...
template <typename T>
_srl_node<T>::_srl_node(T arg_data) {
	data = arg_data;
	_srl_node();
}
...
void srlist<T>::push_back(T elem) {
	...
	_srl_node<T>* _end_node = _tail->_srl_children[0];
	int i = 0;
...
template <typename T>
void srlist<T>::print() {
	_srl_node<T> *_it = _head->_srl_children[0];;
	_it->color = 0;
	int i = 0;
...
public: 
	...
	void push_back(T elem) {
		std::cout << "Start pushing " << elem << std::endl;
		_srl_node<T>* _new_node = new _srl_node(elem);
		_tail->color = 0; // mark the tail
		_srl_node<T>* _end_node = _tail->_srl_children[0];
		int i = 0;
		if (_end_node._srl_children[0]->color != 0) i = 1;
